...
def test_load_mock_called(tmp_path_factory):
    ...
    odem_proc = odem.ODEMProcessImpl(record, fixture_configuration(), workdir,
                                     str(log_dir), odem.get_worker_logger(str(log_dir)))
...
def test_odem_process_identifier_local_workdir(tmp_path):
    ...
    # arrange
    work_dir = tmp_path / 'workdir' / 'foo_bar'
    work_dir.mkdir(parents=True, exist_ok=True)

    # act
    odem_proc = odem.ODEMProcessImpl(None, configuration=None, work_dir=work_dir)
...
def _fixture_odem_setup(tmp_path):
    ...
    cfg.read(os.path.join(PROJECT_ROOT_DIR, 'resources', 'odem.worker-example.ini'))
...
def test_module_fixture_one_integrated_ocr_in_mets(fixture_27949: odem.ODEMProcessImpl):
    ...
    * contain required link data to images
    ...
    root = ET.parse(fixture_27949.mets_file_path).getroot()
    phys_links = root.xpath('//mets:div[@TYPE="physSequence"]/mets:div', namespaces=df.XMLNS)
    # at most 2: one MAX-Image plus according optional FULLTEXT
    assert len(phys_links[1].getchildren()) == 1
    assert len(phys_links[2].getchildren()) == 2
    assert len(phys_links[3].getchildren()) == 2
    assert len(phys_links[4].getchildren()) == 2
    assert len(phys_links[5].getchildren()) == 2
    assert len(phys_links[6].getchildren()) == 1
...
def test_images_4_ocr_properly_filtered(tmp_path):
    ...
    max_dir.mkdir()
    for i in range(1, 6):
        file_path = f"{max_dir}/{i:08d}.jpg"
        with open(file_path, 'wb') as tmp_img_writer:
    ...
    odem_processor = odem.ODEMProcessImpl(record, fixture_configuration(), work_dir,
                                          logger=odem.get_worker_logger(str(log_dir)))
    # act
...
def test_no_catch_when_load_exc(mock_load, tmp_path):
    ...
    log_dir = tmp_path / 'log'
    log_dir.mkdir()
    odem_proc = odem.ODEMProcessImpl(record,
                                     fixture_configuration(),
                                     work_dir,
                                     log_dir=log_dir)
...
@unittest.mock.patch("digiflow.ContainerDerivansManager.start")
@unittest.mock.patch("digiflow.ContainerDerivansManager.init")
def test_expected_pdf_missing(mock_derivans_init: unittest.mock.Mock,
                              mock_derivans_start: unittest.mock.Mock,
                              tmp_path):
    """Behavior when Derivans executed but PDF missing"""
    ...
    with pytest.raises(odem.ODEMDerivateException) as odem_exc:
        oproc.create_derivates()

    # assert
    assert mock_derivans_init.called
    assert "PDF missing in" in odem_exc.value.args[0]


@unittest.mock.patch("digiflow.ContainerDerivansManager.start")
@unittest.mock.patch("digiflow.ContainerDerivansManager.init")
def test_expected_pdf_zero_bytes(mock_derivans_init: unittest.mock.Mock,
                                 mock_derivans_start: unittest.mock.Mock,
                                 tmp_path):
    """Behavior when Derivans creates ZERO byte PDF"""
    ...
    oproc.mets_file_path = str(trgt_mets)
    mock_derivans_start.return_value = df.DerivansResult("dummy", 1.0, "foo")
    with open(path_workdir / "058141367.pdf", mode="wb") as pdf_writer:
        pdf_writer.write(b"")
    ...
    # assert
    assert mock_derivans_init.called
    assert "PDF too small: 0B for " in odem_exc.value.args[0]
